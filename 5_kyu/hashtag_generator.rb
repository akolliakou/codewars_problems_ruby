...
# Here's the deal:
# It must start with a hashtag (#).All words must have their first letter 
# capitalized.If the final result is longer than 140 chars it must return 
# false.If the input or the result is an empty string it must return false.
...
def generateHashtag(str)
  clean_str = str.squeeze(" ").split.map(&:capitalize).join(" ")

  if clean_str.empty? || clean_str.length >= 140
    false
  else
    clean_str.delete(" ").prepend('#')
  end
