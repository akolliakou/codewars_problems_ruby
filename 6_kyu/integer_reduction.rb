...
# Let's take an example of `solve(123056,4)`. We need to remove `4` digits from `123056` and return 
# the lowest possible number. The best digits to remove are `(1,2,3,6)` so that the remaining digits 
# are `'05'`. Therefore, `solve(123056,4) = '05'`.

# Note also that the order of the numbers in `n` does not change: `solve(1284569,2) = '12456',` because 
# we have removed `8` and `9`.

def solve(num, int)
  removed = num.digits.reverse.combination(num.digits.length - int).to_a
  removed.min.join
end